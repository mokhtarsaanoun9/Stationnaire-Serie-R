...
#Import des données
chemin_fichier <- "C:\\Users\Mokhtar\\Downloads\\SerieProduit.csv"


serie=read.csv(chemin_fichier)
View(serie)
...
acf(Trend) 
# ( On conserve les valeurs pour 0<=q<=1)
pacf(Trend) # (On garde les valeurs pour 0<=p<=1 )

# Calcul de l'ACF et du PACF pour la série temporelle 'Trend'
acf_values <- acf(Trend)
pacf_values <- pacf(Trend)

# Affichage des graphiques de l'ACF et du PACF
par(mfrow=c(1,2))  # Met en place une disposition à une ligne et deux colonnes
plot(acf_values, main="Autocorrelogramme", col="blue")
plot(pacf_values, main="Autocorrelogramme Partiel", col="red")
